...
- **`<C-a>` と基数**: `'nrformats'` で `0b10` `0777` `0x1a` を 2/8/16 進と認識するか制御。実用上の既定は両者とも **`bin,hex`**（現代の Vim は vimrc 不在時に自動で `defaults.vim` が適用され `bin,hex` に、Neovim はそもそも `bin,hex`）。`0777` は**10 進**として扱われる。raw Vim 既定（`vim -u NONE`）は `bin,octal,hex` で `0777` を 8 進扱いするが、ほとんどのユーザはこちらは見ない。8 進解釈が真に必要なら `octal` を明示追加
